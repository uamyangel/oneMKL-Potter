...
class xcvupDeviceData:
    ...
    def __init__(self):
        ...
        self.pips =  [
            ...
            (re.compile(r'DELAYWIRE.*'),             0),
            (re.compile(r'BYPASS.*'),                      0),
            (re.compile(r'IMUX_.*'),             0),
            # (re.compile(r'EASTBUSIN_.*'),             0),
            # (re.compile(r'EASTBUSOUT_.*'),             0),
            # (re.compile(r'IMUX_FT.*'),             0),
            # (re.compile(r'CTRL_.*'),             0),
            # (re.compile(r'CFRM_CBRK_.*'),             0),
            # (re.compile(r'BYPASS_.*'),             0),
            # (re.compile(r'INT_INTF_.*'),             0),
            # (re.compile(r'NULL_.*'),             0),
            # (re.compile(r'XIL_UCONN_.*'),             0),
            # (re.compile(r'IO_TO_CTR_.*'),             0),
            # (re.compile(r'NULL'),             0),
            # LAG_LAG tiles
        ...
        self.tile_types = {
            ...
            'GTY_R', 'LAG_LAG', 
            'RCLK_INT_L', 'RCLK_INT_R', "RCLK_DSP_INTF_L", "RCLK_DSP_INTF_R", 
            "RCLK_CLEM_L", "RCLK_CLEM_R", "RCLK_LAG_L", "RCLK_LAG_R",
            "RCLK_DSP_INTF_CLKBUF_L", "RCLK_DSP_INTF_CLKBUF_R",
			"RCLK_BRAM_INTF_TD_L", "RCLK_BRAM_INTF_TD_R",
			"RCLK_BRAM_INTF_TD_L", "RCLK_BRAM_INTF_TD_R",
            "RCLK_CLEM_CLKBUF_L", "RCLK_CLEM_CLKBUF_R",
			"RCLK_BRAM_INTF_L", "RCLK_BRAM_INTF_R",
